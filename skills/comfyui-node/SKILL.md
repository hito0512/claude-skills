...
14. **禁止硬编码魔法值**：代码中**禁止**散落的字面量数值/字符串（如 `320`、`250`、`'450px'`、`max=8192` 等反复出现的值）。所有这类常量必须提取为**命名常量**集中定义（Python 用模块级 `UPPER_CASE` 常量；JS 在作用域顶部定义 `const UPPER_CASE = ...`），后续全部引用常量名，**不要写裸值**。改一个值只改常量定义一处，不要在多处重复写同一个字面量。例外：一次性、语义自明且不会复用的值（如 `range(50)` 里的 50 若是 `MAX_OUTPUTS` 仍应提常量）仍提常量；真正只出现一次且含义明确的可豁免
...
- [ ] **无硬编码魔法值**：反复出现的数值/字符串提取为命名常量集中定义，代码里只引用常量名

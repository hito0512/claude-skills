...
- 任何删除/移除操作（`remove`、`delete`、`rename`、`update`、覆盖写入）**必须**先询问用户，得到明确同意后才可执行
- 禁止在未告知用户的情况下自动执行任何破坏性操作
...
- 导出 Markdown
- 查看待办 / 任务列表
- **生成任务卡片 / 导出任务卡片 / 制作卡片** — 用户提到将文档/笔记转为 DynamicTodo 任务卡片时触发。将文档中的清单/笔记解析为 DynamicTodo 可导入的 JSON 文件
  - 写法示例：`sy 20211103171034-f14h747 制作卡片`、`思源 制作卡片 20211103171034-f14h747`、`sy 导出为卡片 20211103171034-f14h747`

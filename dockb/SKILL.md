---
name: dockb
description: 本地文件知识库管理工具（kb CLI）。管理 Excel/PDF/Word/MD/TXT 等文档，搭建个人知识库。支持全文搜索、MCP 服务、多库联邦。Excel 输出表格格式。索引文件存放于 E:\workspace\recordhub\.lore，源文件可分布在任意路径。
---
...
编辑 `E:\workspace\recordhub\.lore\lore.yaml`，使用**绝对路径**指向源文件目录：
...
> 索引数据统一存放在 `E:\workspace\recordhub\.lore\store`。
